...
// WithMinmaxConfig sets MinMax configuration
//
// Usage example:
//
//	client := mcp.NewClient(mcp.WithMinmaxConfig("sk-xxx"))
func WithMinmaxConfig(apiKey string) ClientOption {
	return func(c *Config) {
		c.Provider = ProviderMinmax
		c.APIKey = apiKey
		c.BaseURL = DefaultMinmaxBaseURL
		c.Model = DefaultMinmaxModel
	}
}

...
### 回撤强平（Drawdown Close）是怎么触发的？
当前仅 **Binance** 实盘生效，触发逻辑是“**基准利润阈值 + 基准回撤阈值 + 监控态**”：

1. 先计算目标盈利（按止盈价）  
2. 当当前盈利达到 `drawdown_close_progress_pct` 对应的基准盈利时，进入监控态  
3. 之后按 `drawdown_close_pct` 计算基准回撤，跌到触发线就强平（全平）

示例：开仓 100、止盈 110、杠杆 10x  
- 目标盈利 = 100%  
- 若进度阈值=40%，则基准盈利=40%  
- 若回撤阈值=40%，则允许回撤=40%×40%=16%  
- 触发线=40%-16%=24%  
- 即：先到 +40% 进入监控，回落到 +24% 触发强平

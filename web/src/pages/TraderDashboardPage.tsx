...
function formatOrderAge(ageSeconds?: number): string {
...
export function TraderDashboardPage({
    ...
    useEffect(() => {
        ...
        const loadPendingOrders = async () => {
            ...
            try {
                const orders = await api.getPendingOrders(selectedTraderId)
                if (isActive) {
        ...
        loadPendingOrders()
        const intervalId = setInterval(loadPendingOrders, 10000)
        return () => {
            isActive = false
    ...
    const handleCancelPendingOrder = async (order: PendingOrder) => {
        if (!selectedTraderId) return
        if (!order.order_id && !order.client_id) {
            notify.error(language === 'zh' ? '缺少订单ID，无法撤销' : 'Missing order ID, cannot cancel')
            return
        }

        const orderRef = order.order_id || order.client_id || order.symbol
        const confirmMsg =
            language === 'zh'
                ? `确定要撤销 ${order.symbol} 的挂单吗？`
                : `Are you sure you want to cancel the pending order for ${order.symbol}?`

        const confirmed = await confirmToast(confirmMsg, {
            title: language === 'zh' ? '确认撤销' : 'Confirm Cancel',
            okText: language === 'zh' ? '确认' : 'Confirm',
            cancelText: language === 'zh' ? '取消' : 'Cancel',
        })
        ...
        setCancelingOrderId(orderRef)
        try {
            await api.cancelPendingOrder(
                selectedTraderId,
                order.symbol,
                order.order_id,
                order.client_id
            )
            notify.success(language === 'zh' ? '撤销成功' : 'Order cancelled')
            const refreshed = await api.getPendingOrders(selectedTraderId)
            setPendingOrders(refreshed)
        } catch (err: unknown) {
            const errorMsg =
                err instanceof Error
                    ? err.message
